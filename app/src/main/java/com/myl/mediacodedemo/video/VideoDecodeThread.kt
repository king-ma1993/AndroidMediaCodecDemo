...
class VideoDecodeThread : Thread() {
    companion object {
        ...
        private const val DECODE_TIMEOUT = 1000L
    }
    ...
    override fun run() {
        ...
        while (isStop.not()) {
            decoder.dequeueInputBuffer(DECODE_TIMEOUT).takeIf { it >= 0 }?.let { index ->
            ...
            when (val outIndex = decoder.dequeueOutputBuffer(newBufferInfo, DECODE_TIMEOUT)) {
